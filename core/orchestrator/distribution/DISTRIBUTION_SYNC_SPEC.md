...
Rule files are validated with `scripts/validate_distribution_sync_rules.py`
against `core/orchestrator/distribution/schemas/sync-rule.schema.json`.
...
sync_rule:
  ...
  validation:
    ...
    link_integrity: true              # Verify rewritten links still resolve
    secret_scan: true                 # Detect credentials and internal-only tokens
    version_tag: true                 # Verify expected release/version tag is present
```
...
**Execution status:** {dry-run-only|disposable-local-real-io|real-downstream-repo-io}
**Evidence manifest:** {downstream-sync-evidence-manifest-path-or-link}
...
- Unchanged: {unchanged-count} files
- Excluded: {excluded-count} files

### Changed Files

{changed-files-or-manifest-link}
...
- Rationale: {why-this-sync-is-needed}
- Source provenance: {source-release-commit-and-rule-ref}
- Downstream impact: {consumer-impact-summary}
- Evidence artifacts: {links-or-artifact-paths}

### Risk Triggers

- Validation failures: {none-or-list}
- Manual conflicts: {none-or-list}
- Override use: {none-or-list}
- Internal-token rewrites: {none-or-list}
- Missing version tag: {yes/no}
- Public/private boundary changes: {none-or-list}
- Downstream CI failures: {none-or-list}

### Autonomy Budget

- Auto-merge allowed: {yes/no}
- Maximum retry attempts: {count}
- Required reviewer class: {reviewer-class}
- Manual stop conditions: {conditions}

### Rollback Plan

- Previous sync point: {commit-or-version}
- Revert command or rollback PR plan: {command-or-plan}
- Downstream decision log: {path-or-link}
- Upstream follow-up issue rule: {issue-rule}

### Downstream Evidence

- Branch: {branch-link}
- Pull request: {pr-link}
- CI/check run: {ci-link}
- Reviewer decision: {reviewer-link-or-status}
...
## Downstream Evidence Manifest

Phase 09 BHS-100 claims require a machine-readable evidence manifest in addition to PR prose. The manifest is validated with:

```bash
python scripts/validate_distribution_sync_evidence.py --require-real-downstream path/to/downstream-sync-evidence.yaml
```

The schema lives at `core/orchestrator/distribution/schemas/downstream-sync-evidence.schema.json`. It records source provenance, sync-rule metadata, diff counts, validation results, risk triggers, autonomy budget, rollback plan, and downstream branch/PR/CI/reviewer evidence.

`execution_status` is load-bearing:

| Value | Meaning | BHS-100 eligible |
|-------|---------|------------------|
| `dry-run-only` | The sync engine only simulated changes. | No |
| `disposable-local-real-io` | The sync wrote to a local disposable or staging directory. | No |
| `real-downstream-repo-io` | The sync wrote to a real downstream repository branch and produced PR/check/review evidence. | Yes, if all required downstream evidence fields are present. |

For `real-downstream-repo-io`, the validator requires:

| Evidence field | Required proof |
|----------------|----------------|
| Branch | Remote-created branch status and branch URL. |
| Pull request | Open, merged, or closed PR status and PR URL. |
| CI/check run | Completed check-run URL with success, failure, cancelled, or skipped status. Failures must also be listed under risk triggers. |
| Reviewer decision | Approved, changes-requested, or commented reviewer decision status and review URL. |

Real-downstream evidence must not use placeholder repositories such as
`example/downstream-repo`. The branch, PR, CI/check, and reviewer URLs must be
`https://github.com/...` URLs for the manifest's `rule.target_repo`; the
validator checks the expected GitHub URL shape for each evidence kind.

Local-only evidence may validate without `--require-real-downstream`, but it must not be used to claim Phase 09 BHS 100.
...
All downstream repos consuming AGENT-33 assets are tracked here. The table below is the required registry shape; it is not current evidence that those example repositories have active Phase 09 sync PRs.

| Repo | Sync Rule ID | Strategy | Frequency | Last Sync | Version | Status |
|------|-------------|----------|-----------|-----------|---------|--------|
| `example/downstream-repo` | `sync-example-orchestrator-docs` | copy | manual | none recorded | none recorded | Example fixture only |
